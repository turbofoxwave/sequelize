...
var QueryGenerator = {
  ...
  createFunction: function(functionName, params, returnType, language, body, options) {
    if(!returnType) throw new Error("createFunction requires returnType");
    if(!language)   throw new Error("createFunction requires language");
    if(!body)       throw new Error("createFunction requires body");
...
expandFunctionParamList: function expandFunctionParamList(params) {
    ...
    var paramList =[]
    Utils._.each(params, function expandParam(curParam) {
      var paramDef = [];
      ...
      if (Utils._.has(curParam, 'type')) {
        ...
        throw new Error('parameter missing type');
      }

      var joined = paramDef.join(' ')
      return joined ?  paramList.push(joined): null
    });
